...
- 紧凑文件行：行高从 93–124px 收敛到 55–72px（状态/重试并入右侧列、图标跨两行居中）。
- 属性栏检查器补全高度（1440×960 下 284×870 通栏）。
- 双分辨率真实桌面 QA（Electron + 真实后端，种子数据 NOTE/MD/TXT/PDF）：
  - 1440×960：导航 212 / 列表 320 / 正文 486 / 检查器 284，正文为最大区域；无横向溢出；图标 20×20；类型徽章 PDF(红)/TXT(橙)/MD(绿)/NOTE(品牌绿) 与真实状态（失败/已完成）。
  - 1080×720：按规格自动收起导航与检查器、保留资料列表；无横向溢出。
  - 交互验证：直接编辑输入出现“有未保存的修改”保存条；自定义分类下拉展开 3 项、选中对勾、点击外部/Esc 关闭。
- 截图存档：docs/audits/round16-knowledge-light-1440x960.png、round16-knowledge-light-1080x720.png。
