...
进行中（阶段追踪已于 2026-08-22 落地，见上方进度记录）。
...
- **求职计划页产品化第二轮（按用户反馈逐项落实）**：① 头部左移出边距、新建按钮内收；② 当前阶段节点下显示进入时间；③ 近期面试记录扩容至 5 条并加大行距、附轮次/题量元信息；④ 公司标识接入 Simple Icons 真实图标（构建期打包 15 家，运行时零网络请求，未收录公司回退品牌色字母标识）；⑤ 新增计划工具栏：阶段筛选（含计数）+ 岗位分类筛选（后端/前端/客户端/测试/算法/数据/产品，关键词推导）；⑥ 投递工具：卡片「投递 ↗」打开弹窗，可编辑行业/期望岗位/地点/备注（后端新增 PATCH /projects/{id}/application，迁移 V26/V6 四列），内置 20 家大厂招聘官网目录支持搜索直达；⑦ 卡片底部展示地点/行业。验证：后端 JobProject 14/14、前端 build 通过、目标页测试 9/9。隐私说明见 privacy-and-ai.md 第三方素材一节。
