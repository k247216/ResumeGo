...
## 2026-09-10｜移动端「职达 · 移动端」定位为独立本地记录工具

**决定**

`ResumeGo/mobile` 是与桌面 EXE 独立运行的纯本地 Android 应用（Vue 3 + Vite + Capacitor + 手写设计系统），只覆盖求职目标、日程提醒、简历记录三块，不与桌面端共享数据库或云同步。简历模块被重新定义为**导入文件型**：用户从本机导入自己的 `.md` / `.pdf`，每次上传即一个版本，支持切换版本、只读预览、原文件转发给 HR 并绑定到某个求职目标；不再内置结构化简历编辑，也不再由内置内容渲染导出 PDF。日程首页聚焦“下一场”倒计时作为首要行动，选择类控件统一改为底部滑动选择器。

**原因**

移动端定位为轻量记录工具，不是网页版简历助手；结构化简历在本机既不可编辑也无真实内容，导出链路是伪功能。以“用户自己的文件”为唯一真相可去掉编造内容的风险、贴合转发给 HR 的真实场景，并与桌面 V2 首页“聚焦下一场”的既有方向一致。数据模型因此发生破坏性变更，本地存储键升级到 `zhida-mobile-db-v2`。

**影响**

- 简历版本二进制存 IndexedDB，JSON 备份仅含文本记录、不含简历文件本体，换机后需重新导入（UI 已提示）；
- 移除 `html2canvas` / `jspdf` 与移动端 `types/resume.ts` 结构化类型；示例数据不再注入任何虚构简历；
- 语义 token、阶段规则与公司品牌图标仍与桌面端同名复用，保证同一产品家族，但移动端不沿用桌面左侧栏布局；
- 后续若引入云同步、真实简历解析或 AI 改写，须另行更新隐私说明并重新评估非目标边界。
