...
| --- | --- | --- | --- |
| [相对路径] | [Create / Modify / Test] | [具体变更] | [是 / 否；否时必须在第 12 节记录，并附 Core Controller 重新授权或新任务卡引用] |
...
| --- | --- | --- | --- |
| [通过 `git status --short` 发现的改动；没有则写“无”] | [是 / 否] | [授权 / 非授权 / 无] | [重新授权/新任务卡引用；没有授权时写明已停止、已移交或已从提交移除] |
...
- [ ] 最终提交相对 `Base commit` 的改动均在任务卡 `Allowed files` 内；如存在额外文件，第 12 节已附 Core Controller 的重新授权或新任务卡引用。仅解释原因不构成授权，也不能勾选本项。
