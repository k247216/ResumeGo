  ...
  <div>
    <header class="page-head">
      <div class="grow">
        ...
        <p class="page-sub">上传你的 PDF / Markdown · {{ resumes.length }} 份</p>
      </div>
      <button class="icon-btn" aria-label="导入简历" :disabled="busy" @click="openPicker">
        <AppIcon name="upload" :size="18" />
      </button>
    </header>
    <input ref="fileRef" type="file" accept=".pdf,.md,.markdown,.txt,application/pdf,text/markdown,text/plain" hidden @change="onFile">

    <!-- 首次使用：导入引导 -->
    <div v-if="!resumes.length" class="import-hero card">
      <div class="ih-art"><AppIcon name="file" :size="30" /></div>
      <h3>还没有简历</h3>
      <p>把你现有的简历文件传进来就能管理。之后每次改好新版，<br>再传一次即可，旧版本会完整保留。</p>
      <button class="btn-primary block" :disabled="busy" @click="openPicker">
        <AppIcon name="upload" :size="17" /> {{ busy ? '导入中…' : '选择 PDF 或 Markdown 文件' }}
      </button>
