...
describe('linkResumeVersionToTarget', () => {
  it('preserves the target job while linking a newly created resume version', async () => {
    vi.mocked(getProject).mockResolvedValue({ success: true, data: { id: 4, name: '目标', status: 'active', jobDescriptionId: 9, resumeVersionId: null, archivedAt: null, createdAt: '', updatedAt: '' } })
    vi.mocked(updateProjectLinks).mockResolvedValue({ success: true, data: { id: 4, name: '目标', status: 'active', jobDescriptionId: 9, resumeVersionId: 12, archivedAt: null, createdAt: '', updatedAt: '' } })
