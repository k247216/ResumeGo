...
export function useInterviewComposer() {
  ...
  function buildRequest(): CreateInterviewPlanRequest {
    if (mode.value === 'ROLE_BASED') {
      const draft = roleDraft.value
  ...
  async function start(): Promise<InterviewPlanResponse> {
    if (mode.value == null) {
    ...
    try {
      const response = await createInterviewPlan(buildRequest())
