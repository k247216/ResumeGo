  ...
  <div class="compare-toolbar" data-test="compare-toolbar">
    <div class="compare-left">
      ...
      <template v-else>
        ...
        >{{ comparing ? `隐藏 V${parentVersionNo} 对比` : `显示 V${parentVersionNo} 对比` }}</button>
...
const emit = defineEmits<{
  'update:comparing': [value: boolean]
}>()
...
<style scoped>
.compare-toolbar{display:flex;align-items:center;gap:12px;width:100%;flex:0 0 auto}
.compare-left{display:flex;align-items:center;gap:10px;min-width:0}
.compare-toggle{border:1px solid var(--line,rgba(28,31,35,.18));border-radius:8px;background:transparent;color:var(--copy);padding:5px 11px;font-size:11.5px;font-weight:550;cursor:pointer;transition:all .14s ease-out}
